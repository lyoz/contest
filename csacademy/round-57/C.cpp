...
int main()
{
	...
	for(int n,k;cin>>n>>k&&n|k;){
		vi fs(k),hs(k);
		rep(i,k) cin>>fs[i],fs[i]--;
		rep(i,k) cin>>hs[i],hs[i]--;

		sort(all(fs)); sort(all(hs));

		Graph g(2*k);
		rep(i,k){
			int l=lower_bound(all(hs),fs[i]-1)-begin(hs);
			int r=upper_bound(all(hs),fs[i]+1)-begin(hs);
			repi(j,l,r) g[i].emplace_back(i,k+j);
		}
		if(fs[0]==0&&hs[k-1]==n-1)
			g[0].emplace_back(0,k+k-1);
		if(fs[k-1]==n-1&&hs[0]==0)
			g[k-1].emplace_back(k-1,k);

		vi side(2*k),match(2*k);
		fill(begin(side)+k,end(side),1);
		cout<<2*k-HopcroftKarp(g,side,match)<<endl;
	}
}
