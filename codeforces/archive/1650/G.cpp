...
int main()
{
	...
	rep(_,tc){
		...
		map<tuple<int,int>,mint> q; // ((distance,vertex),count)
		q[{0,s}]=1;
		vector<vector<tuple<int,mint>>> dists(n); // [(distance,count)]
		while(q.size()){
			mint c=begin(q)->second;
			auto [d,u]=begin(q)->first;
			q.erase(begin(q));
			if(dists[u].size()==2)
				continue;
			dists[u].emplace_back(d,c);
			for(int v:g[u])
				q[{d+1,v}]+=c;
		}

		auto [d1,c1]=dists[t][0];
		mint res=c1;
		if(dists[t].size()==2)
			if(auto [d2,c2]=dists[t][1];d1+1==d2)
				res+=c2;
		cout<<res<<endl;
	}
}
