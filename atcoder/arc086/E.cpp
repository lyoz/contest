...
int get_size(const vvi& g,int u,vi& ss)
{
	int res=1;
	for(int v:g[u]) res+=get_size(g,v,ss);
	return ss[u]=res;
}

mint dfs(const vvi& g,const vi& ss,int u,int step,vector<vector<mint>>& memo)
{
	mint& res=memo[u][step];
	if(memo[u][step]==mint(-1)){
		if(step==0)
			res=mint(2).pow(ss[u]-1);
		else{
			res=0;
			mint mul=1;
			for(int v:g[u])
				mul*=mint(2).pow(ss[v])-dfs(g,ss,v,step-1,memo);
			for(int v:g[u])
				res+=mul/(mint(2).pow(ss[v])-dfs(g,ss,v,step-1,memo))*dfs(g,ss,v,step-1,memo);
			res*=2;
		}
	}
	return res;
...
int solve_subtask(vvi g)
{
	int n=g.size();
	vi ss(n);
	get_size(g,0,ss);
	vector<vector<mint>> memo(n,vector<mint>(n,-1));
	rep(i,n) dfs(g,ss,0,i,memo);
	mint res=0;
	rep(i,n) res+=memo[0][i];
	return res.val;
}
