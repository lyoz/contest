...
int main()
{
	...
	for(int n,m;cin>>n>>m&&n|m;){
		vvvmi dp(m+2,vvmi(m+2,vmi(m+2)));
		dp[m+1][m+1][m+1]=1;
		rep(_,n){
			vvvmi dp2(m+2,vvmi(m+2,vmi(m+2)));
			repi(i,1,m+2) repi(j,1,m+2) repi(k,1,m+2) repi(x,1,m+1){
				if(k<x)
					continue;
				int ni=min(i,x);
				int nj=i<x?min(j,x):j;
				int nk=j<x?min(k,x):k;
				dp2[ni][nj][nk]+=dp[i][j][k];
			}
			dp=dp2;
		}
		mint sum=0;
		rep(i,m+1) rep(j,m+1) rep(k,m+1)
			sum+=dp[i][j][k];
		cout<<sum<<endl;
	}
}
