...
int main()
{
	...
	for(int b,w;cin>>b>>w&&b|w;){
		vector<mint> ps(b+w),qs(b+w);
		rep(i,b+w){
			ps[i]=i<b?0:ps[i-1]+choose(i-1,b-1)/mint(2).pow(i);
			qs[i]=i<w?0:qs[i-1]+choose(i-1,w-1)/mint(2).pow(i);
			mint res=(mint(1)-ps[i]-qs[i])/mint(2)+qs[i];
			cout<<res<<endl;
		}
	}
}
