...
struct FenwickTree{
	...
	void AddRange(int i,mint x){
		for(;i;i-=i&-i)
			data[i]+=x;
	}
	void AddRange(int i,int j,mint x){
		AddRange(i,0-x);
		AddRange(j,x);
	}
	mint Get(int i){
...
int main()
{
	...
	for(int n,m,k;cin>>n>>m>>k&&n|m|k;){
		vector<FenwickTree> dp(n,m+1);
		dp[0].AddRange(1,m+1,1);
		rep(i,n-1){
			repi(j,1,m+1){
				mint x=dp[i].Get(j);
				if(k==0){
					dp[i+1].AddRange(1,m+1,x);
				}
				else{
					// [1...j-k]
					if(1<=j-k)
						dp[i+1].AddRange(1,j-k+1,x);
					// [j+k...m]
					if(j+k<=m)
						dp[i+1].AddRange(j+k,m+1,x);
				}
			}
		}
		mint res=0;
