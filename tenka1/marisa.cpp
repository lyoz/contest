...
int whose(int x,int y)
{
	if(abs(x-15)+abs(y-15)<=10) // (15,15)を中心とする対角線20の正方形領域
		return 4;
	if(x<15&&y<15)
		return 0;
	if(15<=x&&y<15)
...
int main()
{
	...
	for(;;){
		...
		rep(ai,game.agents.size()){
			...
			if(whose(cur.x,cur.y)!=ai){
				const auto [nx,ny]=centers[ai];
				cerr<<"# agent "<<ai+1<<": ("<<cur.x<<','<<cur.y<<")->("<<nx<<','<<ny<<')'<<"(init)"<<endl;
